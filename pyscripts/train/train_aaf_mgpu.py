...
def main():
  ...
  restore_var = [
    v for v in tf.global_variables()
      if ('block5' not in v.name or not args.not_restore_classifier) and 'edge' not in v.name
  ]
  ...
  for outputs, labels in zip(outputs_mgpu, labels_mgpu):
    with tf.device(labels.device):
      ...
      for i,output in enumerate(outputs):
        ...
        aaf_loss = tf.reduce_mean(eloss_1)*(args.kld_lambda_1*dec*fac)
        aaf_loss += tf.reduce_mean(eloss_2)*(args.kld_lambda_1*dec*fac)
        aaf_loss += tf.reduce_mean(eloss_3)*(args.kld_lambda_1*dec*fac)
        aaf_loss += tf.reduce_mean(neloss_1)*(args.kld_lambda_2*dec*fac)
        aaf_loss += tf.reduce_mean(neloss_2)*(args.kld_lambda_2*dec*fac)
        aaf_loss += tf.reduce_mean(neloss_3)*(args.kld_lambda_2*dec*fac)
  ...
  base_trainable = [v for v in all_trainable if 'block5' not in v.name and 'edge' not in v.name] # lr*1
  ...
  with tf.device('/cpu:0'):
    ...
    aaf_loss_summary = tf.summary.scalar(
        'aaf_loss', mean_aaf_loss)
