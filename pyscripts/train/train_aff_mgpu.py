...
def main():
  ...
  for outputs, labels in zip(outputs_mgpu, labels_mgpu):
    with tf.device(labels.device):
      ...
      for i,output in enumerate(outputs):
        ...
        aff_loss = tf.reduce_mean(edge_loss)*(args.kld_lambda_1*dec*fac)
        aff_loss += tf.reduce_mean(not_edge_loss)*(args.kld_lambda_2*dec*fac)
        mean_aff_losses.append(aff_loss)
  ...
  mean_aff_loss = tf.add_n(mean_aff_losses)
  mean_l2_loss = tf.add_n(mean_l2_losses)
  reduced_loss = mean_seg_loss+mean_aff_loss+mean_l2_loss
